...
// Email templates
export function getVerificationEmailTemplate(token: string, appUrl: string, redirectUrl?: string): EmailOptions {
  const verificationUrl = redirectUrl 
    ? `${appUrl}/auth/verify-email?token=${token}&redirect=${encodeURIComponent(redirectUrl)}`
...
export function getMagicLinkEmailTemplate(magicLinkUrl: string): EmailOptions {
  return {
    to: "", // Will be set by caller
